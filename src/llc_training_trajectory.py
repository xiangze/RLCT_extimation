...
def estimate_llc_sgld_volume(model: nn.Module,
...
class TrainLog:
    ...
    def append_train(self,e,tr,test,me):
        ...
        self.test_losses.append(test)
        self.metrics.append(me)
    def append_llc(self,llc):
        self.llc_values.append(llc)
    def writecsv(self,csv_path):
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["epoch", "train_loss", "test_loss", "metric", "llc"])
            for e, tr, te, m, l in zip(self.epochs, self.train_losses, self.test_losses, self.metrics, self.llc_values):
                writer.writerow([e, tr, te, m, l])

def detect_plateau(history: List[float], window: int, tol: float) -> bool:
    """
...
def train_evel_step(model,epoch,train_loader,device,optimizer,loss_fn,test_loader,task_type):
    model.train()
    total_loss = 0.0
    total_n = 0
    ...
    print(f"[Epoch {epoch:03d}] "
            ...
            f"metric={metric:.4f}")
    return avg_train_loss,avg_test_loss,metric
...
def train_and_track_llc(cfg: TrainConfig, llc_cfg: LLCConfig):
    ...
    for epoch in range(1, cfg.max_epochs + 1):
        train_loss,test_loss,metric=train_evel_step(model,epoch,train_loader,device,optimizer,loss_fn,test_loader,task_type)
        trainlog.append_train(epoch,train_loss,test_loss,metric)
        ...
        if cfg.llc_mode == "precomputed":
            ...
            if (epoch % cfg.llc_check_interval == 0 and
                    detect_plateau(trainlog.train_losses, cfg.plateau_window, cfg.plateau_tol)):
                print(f"[Epoch {epoch:03d}] Plateau detected, estimating LLC via SGLD...")
                if(llc_cfg.method=="volume"): #not stable
                    llc_val = estimate_llc_sgld_volume(
                        model=model,
                    ...
                    llc_val = power.calc_lambda(parser.parse_args())
                else: #default
                    alpha_list,lambda_list = estimate_softmaxDNN.estimate_lambda( model=model,)
                    llc_val=[alpha_list,lambda_list]
                print(f"[Epoch {epoch:03d}] Estimated LLC (slope) = {llc_val:.4f}")

        trainlog.append_llc(llc_val)
    # Save logs to CSV
    csv_path = os.path.join(cfg.output_dir, "training_llc_log.csv")
    trainlog.writecsv(csv_path)
    ...
    ax1.set_ylabel("loss")
    ax1.plot(trainlog.epochs, trainlog.train_losses, label="train_loss")
    ax1.plot(trainlog.epochs, trainlog.test_losses, label="test_loss", linestyle="--")
    ax1.legend(loc="upper left")

    ax2 = ax1.twinx()
    ax2.set_ylabel("LLC (slope)")
    ax2.plot(trainlog.epochs, trainlog.llc_values, label="LLC", color="tab:red", marker="o")
